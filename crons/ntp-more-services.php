...
$start = time();
if ($staters = APICache::read('ntp1-more-services'))
{
    $staters[] = $start;
    ...
    if (count($starters)>50)
        unset($starters[0]);
    sort($staters, SORT_ASC);
    APICache::write('ntp1-more-services', $staters, 3600 * 24 * 7 * 4 * 6);
    ...
    APICache::write('ntp1-more-services', array(0=>$start), 3600 * 24 * 7 * 4 * 6);
    $seconds = 1800;
}
...
$sql = array();
$prefixes = array('ntp2', 'ntp3', 'ntp4', 'ntp5', 'ntp6', 'ntp7', 'ntp8', 'ntp9', 'ntp10', 'ntp11', 'ntp12', 'ntp13', 'ntp14', 'ntp15', 'ntp16', 'ntp17', 'ntp18', 'ntp19', 'ntp20', 'ntp21', 'ntp22', 'ntp23', 'ntp24', 'ntp25', 'ntp26', 'ntp27', 'ntp28', 'ntp29', 'ntp30', 'ntp31', 'ntp32', 'clock', 'time', '1.ntp', '2.ntp', '3.ntp', '4.ntp', '5.ntp', '6.ntp', '7.ntp', '8.ntp', '9.ntp', '10.ntp', '11.ntp', '12.ntp', '13.ntp', '14.ntp', '15.ntp', '16.ntp', '17.ntp', '18.ntp', '19.ntp', '20.ntp', '21.ntp', '22.ntp', '23.ntp', '24.ntp', '25.ntp', '26.ntp', '27.ntp', '28.ntp', '29.ntp', '30.ntp', '31.ntp', '32.ntp', '0.ntp');
$question = "SELECT * FROM `" . $GLOBALS['APIDB']->prefix('ntpservices') . "` WHERE `hostname` LIKE 'ntp.%' AND `pinging` > 0 AND `uptime` > `downtime` AND `uptime` > 0 ORDER BY RAND()";
...
while($ntpservice = $GLOBALS['APIDB']->fetchArray($result)) {
    ...
    $hostname = str_replace('ntp.', 'ntp1.', $hname);
    ...
    if ($count==0) {
        ...
        if ($ping > false && $now > 0) {
            ...
            foreach($prefixes as $prefix) {                
                $hostname = str_replace('ntp.', $prefix.'.', $hname);
                list($count) = $GLOBALS['APIDB']->fetchRow($GLOBALS['APIDB']->queryF("SELECT count(*) FROM `" . $GLOBALS['APIDB']->prefix('ntpservices') . "` WHERE `hostname` LIKE '$hostname'"));
                if ($count==0) {
                    echo ("\nPinging " . ($hostname = str_replace('ntp.', $prefix.'.', $hname)) . ":" . $ntpservice['port'] . ' ' );
                    if ($now = (float)getTimeFromNTP($hostname, $ntpservice['port']))
                        $ping = getHostnamePing($hostname, $ntpservice['port']);
                    else
                        $ping = 0;
